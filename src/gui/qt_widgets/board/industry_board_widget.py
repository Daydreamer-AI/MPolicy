...
class IndustryBoardWidget(QWidget):
    def __init__(self):
        super().__init__()
        uic.loadUi('./src/gui/qt_widgets/board/IndustryBoardWidget.ui', self)
    ...
    def init_para(self):
        ...
        self.last_board_data_date = self.df_industry_board_data['date'].iloc[0]
        self.logger.info(f"当前行业板块最新数据日期：{self.last_board_data_date}")
    ...
    def init_card_list(self):
        ...
        if df_lastest_industry_data is None or df_lastest_industry_data.empty:
            self.logger.warning("没有获取到行业板块数据！")
            return
    ...
    def showEvent(self, event):
        ...
        new_df_data = AKStockDataProcessor().query_ths_board_industry_data()
        new_data_date = new_df_data['date'].iloc[0]

        self.logger.info(f"初始化保存的行业板块数据日期：{self.last_board_data_date}")
        self.logger.info(f"最新获取的行业板块数据日期：{new_data_date}")

        if self.last_board_data_date != new_data_date:
            self.logger.info(f"行业板块数据已更新，重新加载")

            self.df_industry_board_data = new_df_data
            self.last_board_data_date = new_data_date

            self.init_card_list()
        else:
            self.logger.info(f"已是最新行业板块数据")
