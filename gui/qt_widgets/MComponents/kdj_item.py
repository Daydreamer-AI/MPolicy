...
# 颜色配置
color_table = {
    'k_line': (255, 208, 0),        # K线 - 黄
    'd_line': (23, 50, 157),        # D线 - 蓝
    'j_line': (244, 134, 208),        # J线 - 紫
...
class KDJItem(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        if len(k_points) > 1:
            p.setPen(pg.mkPen(color_table['k_line'], width=2))
        ...
        if len(d_points) > 1:
            p.setPen(pg.mkPen(color_table['d_line'], width=2))
        ...
        if len(j_points) > 1:
            p.setPen(pg.mkPen(color_table['j_line'], width=2))
