...
def compile_qrc_file(qrc_file_path=DEFAULT_QRC_FILE, output_file_path=DEFAULT_OUTPUT_FILE, compiler_type="pyqt5"):
    ...
    if not os.path.exists(qrc_file_path):
        logger.info(f"错误：未找到文件 {qrc_file_path}")
        logger.info(f"请确保文件存在，或使用 -i 参数指定其他路径")
        return False
    ...
    if compiler_type == "auto":
        ...
        for candidate in ["pyrcc5", "pyside6-rcc"]:
            try:
                ...
                compiler = candidate
                logger.info(f"检测到并使用编译器: {compiler}")
                break
            except (subprocess.CalledProcessError, FileNotFoundError):
                continue
        
        if compiler is None:
            logger.info("错误：未找到可用的资源编译器。请确保已安装 PyQt5 或 PySide6。")
            logger.info("您可以尝试使用 -c 参数明确指定编译器类型")
            return False
    else:
        compiler_map = {
            "pyqt5": "pyrcc5",
    ...
    try:
        logger.info(f"正在编译: {qrc_file_path} -> {output_file_path}")
        logger.info(f"执行命令: {' '.join(compile_command)}")
        
        result = subprocess.run(compile_command, check=True, capture_output=True, text=True)
        
        logger.info("✅ 编译成功！")
        if result.stderr:
            logger.info(f"编译器输出: {result.stderr}")
        
        # 验证输出文件是否已创建
        if os.path.exists(output_file_path):
            logger.info(f"输出文件已创建: {output_file_path}")
        else:
            logger.info("⚠️  警告：编译命令执行成功，但未找到输出文件")
        
        return True
        
    except subprocess.CalledProcessError as e:
        logger.info(f"❌ 编译失败！退出码: {e.returncode}")
        if e.stderr:
            logger.info(f"错误信息: {e.stderr}")
        return False
    except Exception as e:
        logger.info(f"❌ 编译过程中发生未知错误: {e}")
        return False
